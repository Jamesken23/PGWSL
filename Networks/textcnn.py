...
class TextCNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_filter, filter_sizes, output_dim=2, dropout=0.5):
        super().__init__()
    ...
    def forward(self, text, perturbation=None):
        ...
        if perturbation is not None and perturbation != 'advtext':
           ...
           emb += perturbation
        # embedded = self.dropout(t_1)  # [batch size, sent len, emb dim]
        embedded = emb.unsqueeze(1)  # [batch size, 1, sent len, emb dim]
