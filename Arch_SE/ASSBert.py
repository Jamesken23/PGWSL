"""
2023 JISA ASSBert: Active and semi-supervised bert for smart contract vulnerability detection
Ref: https://github.com/shibuiwilliam/ActiveSemiSupervisedDeepLearning/blob/master/ActiveLearning.ipynb
https://github.com/modAL-python/modAL/blob/master/modAL/models/learners.py
"""
import torch
...
class Trainer:
    def __init__(self, model, log_set, device=None):
        ...
        self.log_set.info('--------------------2023 JISA ASSBert--------------------')
    ...
    def train_iteration(self, train_u_loader, train_x_loader):
        ...
        for (label_x, label_y, _), (unlab_x, unlab_y, udx) in zip(train_u_loader, train_x_loader):
            ...
            with torch.no_grad():
                if self.device:
                    preds = unlab_outputs.cpu().detach().numpy()
                else:
                    preds = unlab_outputs.detach().numpy()

                if len(preds) > 0:
                    lm = self.getLowMargin(preds)
                    # nlm = np.array(np.array(lm)[:, 0], dtype="int32")
                    for j in lm:
                        jj = int(j[0])
                        pseudo_label = torch.argmax(unlab_outputs[jj])
                        self.epoch_pslab[udx[jj]] = pseudo_label.detach()
    ...
    def getLowMargin(self, proba, bound=0.01):
        ls = []
